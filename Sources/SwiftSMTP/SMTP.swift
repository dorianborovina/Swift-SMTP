...
public struct SMTP {
    ...
    public let transactionLog: [String] = []
    ...
    // Add this new method to handle creating the email content
        private func createEmailContent(_ mail: Mail) -> String {
            var message = mail.headersString + CRLF
            ...
            if let html = mail.html {
                ...
                message += CRLF
                message += Data(mail.text.utf8).base64EncodedString(options: [.lineLength76Characters, .endLineWithCarriageReturn])
                message += CRLF + CRLF
                ...
                message += "Content-Transfer-Encoding: base64\(CRLF)"
                message += CRLF
                message += Data(html.utf8).base64EncodedString(options: [.lineLength76Characters, .endLineWithCarriageReturn])
                message += CRLF + CRLF
